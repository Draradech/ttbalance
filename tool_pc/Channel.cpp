...
#define GLIDING_AVERAGE_TIME 200
...
void CChannel::setValue(signed short value)
{
	int serialTime = m_pTool->getSerialTime();
	int index = serialTime % HISTORY;
...
void CChannel::drawGraph(int width, int height)
{
	...
	int serialTime = m_pTool->getSerialTime();
	int serialTimeRightEdge = serialTime / zoomOut * zoomOut;
	int serialTimeLeftEdge = serialTimeRightEdge - width * zoomOut;
